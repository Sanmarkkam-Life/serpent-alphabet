...
snake's path, and recognize it three times — and the next letter on the
...
2. **Drop in the illustration.** Put the wide (~16:9) snake image at
   ...
   image is missing the app shows the glyph as a graceful fallback —
   nothing breaks.

3. **Record the trace path.** Open the hidden route **`/author`**
   (ideally on a phone — the canvas has the same geometry learners see),
